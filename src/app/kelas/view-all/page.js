            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 justify-center">
              ...
              {kelasList == null && (
                <div className="flex flex-col items-center justify-center h-full">
                  <FaChalkboardTeacher size={64} className="text-gray-400" />
                  <p className="text-lg font-semibold mt-4">Tidak ada kelas yang kamu ikuti</p>
                  <p className="text-sm text-gray-600">Segera buat kelas baru!</p>
                </div>
              )}
              {!loading && !error && kelasList && (
                <>
                  {kelasList.filter(k => k.namaKelas.toLowerCase().includes(query.toLowerCase())).map((kelas) => (
                    <div
                      ...
                      onMouseEnter={() => handleShowDropdown(kelas.idKelas)}
